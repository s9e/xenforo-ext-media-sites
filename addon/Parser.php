...
class Parser
{
	...
	*/
	protected static $sites = [
		...
		'facebook'=>[['@/(?!(?:apps|developers|graph)\\.)[-\\w.]*facebook\\.com/(?:[/\\w]+/permalink|(?!marketplace/|pages/|groups/).*?)(?:/|fbid=|\\?v=)(?<id>\\d+)(?=$|[/?&#])@','@facebook\\.com/(?<user>[.\\w]+)/(?=(?:post|video)s?/)(?<type>[pv])@','@facebook\\.com/video/(?=post|video)(?<type>[pv])@','@facebook\\.com/watch/\\?(?<type>[pv])=@','@facebook.com/groups/[^/]*/(?<type>p)osts/(?<id>\\d+)@','@facebook\\.com/(?<user>[.\\w]+)/posts/pfbid(?<pfbid>\\w+)@'],[],[['extract'=>['@facebook\\.com/(?<user>[.\\w]+)/(?<type>[pv])\\w+/(?<id>\\d+)(?!\\w)@'],'header'=>'User-agent: PHP (not Mozilla)','match'=>['@facebook\\.com/[.\\w]+/posts/pfbid@'],'url'=>'https://www.facebook.com/plugins/post.php?href=https%3A%2F%2Fwww.facebook.com%2F{@user}%2Fposts%2Fpfbid{@pfbid}'],['extract'=>['@facebook\\.com/watch/\\?(?<type>v)=(?<id>\\d+)@','@facebook\\.com/(?<user>[.\\w]+)/(?<type>v)ideos/(?<id>\\d+)@'],'header'=>'User-agent: PHP (not Mozilla)','match'=>['@fb\\.watch/.@']]]],
