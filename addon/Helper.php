...
class Helper
{
	...
	*/
	public static function replaceIframes(Templater $templater, $type, $template, &$output)
	{
		...
		$output .= '<script>((d,g,y,k,t)=>{function r(a){if(g.querySelector(a)){m=!0;z();var b=u;a=g.querySelector(a)?.getBoundingClientRect().top??0;b([a,a+d.innerHeight])}}function z(){d.clearTimeout(A);A=d.setTimeout(()=>{m=!1},200)}function B(a){["click","load","resize","scroll","visibilitychange"].forEach(b=>a(b,I,{capture:!0,passive:!0}))}function C(){return[g.querySelector(".p-navSticky").getBoundingClientRect().bottom??0,d.innerHeight]}function I(a){let b=a.target,c;"click"===a.type&&"A"===b.tagName?(a=b.dataset.contentSelector??"",/^#[-\\w]+$/.test(a)?r(a):(c=/(.*)(#[-\\w]+)$/.exec(b.href))&&c[1]===g.baseURI.replace(/#.*/,"")&&r(c[2])):m&&z();d.clearTimeout(D);D=d.setTimeout(J,32)}function E(a){let b=-1,c=g.createElement("iframe"),e=JSON.parse(a.getAttribute(k+"-iframe"));for(;++b<e.length;)c.setAttribute(e[b],e[++b]);c.loading="eager";K(c,a.parentNode);a.replaceWith(c);2==c.getAttribute(k+"-api")&&(c.onload=()=>L(c),a=l[F(c.src)],"string"===typeof a&&G(c,a))}function L(a){const b=new MessageChannel,c=F(a.src);b.port1.onmessage=e=>{const f=""+e.data;d.setTimeout(()=>{G(a,l[c]||f)},a.getBoundingClientRect().height>+f.split(" ")[0]?4E3:0);try{.1>Math.random()&&M(),l[c]=f}catch{}};a.contentWindow.postMessage("s9e:init","*",[b.port2])}function N(a){a=a.getBoundingClientRect();const [b,c]=C();return a.bottom>c?2:a.top<b?0:1}function G(a,b){const [c,e]=[...b.split(" "),0],f=a.style;if(f.height!==c+"px"||e&&f.width!==e+"px"){a=N(a);var h=(b=0===a||1===a&&1===v&&!m)?p.scrollHeight-d.scrollY:0;if(1!==a||b||m||"complete"!==g.readyState)f.transition="none",d.setTimeout(()=>{f.transition=""},32);q=d.scrollY;f.height=c+"px";e&&(f.width=e+"px");b&&0<q&&(a=p.scrollHeight-d.scrollY-h)&&d.scrollBy({behavior:"instant",top:a});q=d.scrollY}}function J(){v=q>(q=d.scrollY)&&!m?1:0;if("hidden"!==g.visibilityState&&"complete"===g.readyState){let a=2*d.innerHeight;u([-a/(0===v?4:2),a])}}function u(a){const b=[],[c,e]=a;w.forEach(f=>{var h=f.getBoundingClientRect();if(h.bottom<c||h.top>e||!h.width)h=!1;else{let n=f.parentElement,H=n;for(;n;)/bbCodeBlock-expandContent/.test(n.className)&&(H=n),n=n.parentElement;h=h.top<=H.getBoundingClientRect().bottom}h?f.hasAttribute(k+"-c2l")?O(f):E(f):b.push(f)});w=b;w.length||B(d.removeEventListener)}function O(a){a.hasAttribute(k+"-c2l-background")&&((a.hasAttribute(k)?a:a.parentElement.parentElement).style.background=a.getAttribute(k+"-c2l-background"));a.onclick=b=>{b.stopPropagation();E(a)}}function K(a,b){a.hasAttribute(k)||b.hasAttribute("style")||(b.className=t+"-inactive",b.onclick=()=>{const c=b.getBoundingClientRect(),e=a.style;e.bottom=p.clientHeight-c.bottom+"px";e.height=c.height+"px";e.width=c.width+"px";"rtl"===p.dir?e.left=c.left+"px":e.right=p.clientWidth-c.right+"px";a.offsetHeight&&/inactive/.test(b.className)?(b.className=t+"-active-tn",a.removeAttribute("style"),x?.click(),x=b):(b.className=t+"-inactive-tn",x=null)},a.addEventListener("transitionend",()=>{/-tn/.test(b.className)&&(b.className=b.className.replace("-tn",""),a.removeAttribute("style"))}))}function F(a){return a.replace(/.*?ifram(e\\/\\d+\\/\\w+)[^#]*(#[^#]+).*/,"s9$1$2")}function M(){if(l instanceof Storage){var a=l.length||0;if(100<a)for(;0<=--a;){const b=l.key(a)||"";/^s9e\\//.test(b)&&.5>Math.random()&&l.removeItem(b)}}}let x=null,p=g.documentElement,m=!1,q=d.scrollY,l={},A=0,w=[...g.querySelectorAll("span["+k+"-iframe]")],v=0,D=0;try{l=d.localStorage}catch{}y?r(y):u(C());B(d.addEventListener);d.navigation?.addEventListener("navigate",a=>{a=a.destination;a.sameDocument&&(a=/#[-\\w]+$/.exec(a.url))&&r(a[0])})})(window,document,location.hash,"data-s9e-mediaembed","s9e-miniplayer")</script>';
	}
