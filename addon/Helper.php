...
class Helper
{
	...
	*/
	public static function replaceIframes(Templater $templater, $type, $template, &$output)
	{
		...
		$output .= '<script>((d,h,r,k,t)=>{function x(a){["click","load","resize","scroll","visibilitychange"].forEach(b=>a(b,F,{capture:!0,passive:!0}))}function u(){return[Math.max(0,h.querySelector(".p-navSticky")?.getBoundingClientRect().bottom),d.innerHeight]}function F(){d.clearTimeout(y);y=d.setTimeout(z,32)}function A(a){let b=-1,c=h.createElement("iframe"),e=JSON.parse(a.getAttribute(k+"-iframe"));for(;++b<e.length;)c.setAttribute(e[b],e[++b]);c.loading="eager";G(c,a.parentNode);a.replaceWith(c);2==c.getAttribute(k+"-api")&&(c.onload=()=>H(c),a=l[B(c.src)],"string"===typeof a&&C(c,a))}function H(a){const b=new MessageChannel,c=B(a.src);b.port1.onmessage=e=>{const f=""+e.data;d.setTimeout(()=>{C(a,l[c]||f)},a.getBoundingClientRect().height>+f.split(" ")[0]?5E3:0);try{.1>Math.random()&&I(),l[c]=f}catch{}};a.contentWindow.postMessage("s9e:init","*",[b.port2])}function J(a){a=a.getBoundingClientRect();const [b,c]=u();return a.bottom>c?2:a.top<b?0:1}function C(a,b){const [c,e]=[...b.split(" "),0],f=a.style;if(f.height!==c+"px"||e&&f.width!==e+"px"){a=J(a);var g=(b=0===a||1===a&&1===q)?n.scrollHeight-d.scrollY:0;if(1!==a||b||"complete"!==h.readyState)f.transition="none",d.setTimeout(()=>{f.transition=""},32);f.height=c+"px";e&&(f.width=e+"px");b&&((a=n.scrollHeight-d.scrollY-g)&&d.scrollBy({behavior:"instant",top:a}),p=d.scrollY)}}function z(){if("hidden"!==h.visibilityState){q=p===d.scrollY?0:p>(p=d.scrollY)?1:0;var a=D;if("complete"!==h.readyState)var b=u();else b=2*d.innerHeight,b=[-b/(0===q?4:2),b];a(b)}}function D(a){const b=[],[c,e]=a;v.forEach(f=>{var g=f.getBoundingClientRect();if(g.bottom<c||g.top>e||!g.width)g=!1;else{let m=f.parentElement,E=m;for(;m;)/bbCodeBlock-expandContent/.test(m.className)&&(E=m),m=m.parentElement;g=g.top<=E.getBoundingClientRect().bottom}g?f.hasAttribute(k+"-c2l")?K(f):A(f):b.push(f)});v=b;v.length||x(d.removeEventListener)}function K(a){a.hasAttribute(k+"-c2l-background")&&((a.hasAttribute(k)?a:a.parentElement.parentElement).style.background=a.getAttribute(k+"-c2l-background"));a.onclick=b=>{b.stopPropagation();A(a)}}function G(a,b){a.hasAttribute(k)||b.hasAttribute("style")||(b.className=t+"-inactive",b.onclick=()=>{const c=b.getBoundingClientRect(),e=a.style;e.bottom=n.clientHeight-c.bottom+"px";e.height=c.height+"px";e.width=c.width+"px";"rtl"===n.dir?e.left=c.left+"px":e.right=n.clientWidth-c.right+"px";a.offsetHeight&&/inactive/.test(b.className)?(b.className=t+"-active-tn",a.removeAttribute("style"),w?.click(),w=b):(b.className=t+"-inactive-tn",w=null)},a.addEventListener("transitionend",()=>{/-tn/.test(b.className)&&(b.className=b.className.replace("-tn",""),a.removeAttribute("style"))}))}function B(a){return a.replace(/.*?ifram(e\\/\\d+\\/\\w+)[^#]*(#[^#]+).*/,"s9$1$2")}function I(){if(l instanceof Storage){var a=l.length||0;if(100<a)for(;0<=--a;){const b=l.key(a)||"";/^s9e\\//.test(b)&&.5>Math.random()&&l.removeItem(b)}}}let w=null,n=h.documentElement,p=d.scrollY,l={},v=[...h.querySelectorAll("span["+k+"-iframe]")],q=0,y=0;try{l=d.localStorage}catch{}z();x(d.addEventListener);d.navigation?.addEventListener("navigate",a=>{a=a.destination;if(a.sameDocument&&(a=/#.*/.exec(a.url))){r=a[0];q=p=0;a=D;if(r){var b=h.querySelector(r)?.getBoundingClientRect().top??0;b=[b,b+d.innerHeight]}else b=u();a(b)}})})(window,document,location.hash,"data-s9e-mediaembed","s9e-miniplayer")</script>';
	}
