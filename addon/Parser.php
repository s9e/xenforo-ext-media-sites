...
class Parser
{
	...
	*/
	protected static $sites = [
		...
		'facebook'=>[['@/(?!(?:apps|developers|graph)\\.)[-\\w.]*facebook\\.com/(?:[/\\w]+/permalink|(?!marketplace/|pages/|groups/).*?)(?:/|fbid=|\\?v=)(?<id>\\d+)(?=$|[/?&#])@','@facebook\\.com/(?<user>[.\\w]+)/(?=(?:post|video)s?/)(?<type>[pv])@','@facebook\\.com/video/(?=post|video)(?<type>[pv])@','@facebook\\.com/watch/\\?(?<type>[pv])=@','@facebook.com/groups/[^/]*/(?<type>p)osts/(?<id>\\d+)@','@facebook\\.com/(?<user>[.\\w]+)/posts/pfbid(?<pfbid>\\w+)@','@facebook\\.com/permalink\\.php\\?story_fbid=(?:(?<id>\\d+)|pfbid(?<pfbid>\\w+))&id=(?<page_id>\\d+)@'],[],[['extract'=>['@facebook\\.com/(?<user>[.\\w]+)/(?<type>[pv])\\w+/(?<id>\\d+)(?!\\w)@'],'header'=>'User-agent: PHP (not Mozilla)','match'=>['@facebook\\.com/[.\\w]+/posts/pfbid@'],'url'=>'https://www.facebook.com/plugins/post.php?href=https%3A%2F%2Fwww.facebook.com%2F{@user}%2Fposts%2Fpfbid{@pfbid}'],['extract'=>['@story_fbid=(?<id>\\d+)@'],'header'=>'User-agent: PHP (not Mozilla)','match'=>["@facebook\\.com/permalink\\.php\\?story_fbid=pfbid(?'pfbid'\\w+)&id=(?'page_id'\\d+)@"],'url'=>'https://www.facebook.com/plugins/post.php?href=https%3A%2F%2Fwww.facebook.com%2Fpermalink.php%3Fstory_fbid%3Dpfbid{@pfbid}%26id%3D{@page_id}'],['extract'=>['@facebook\\.com/watch/\\?(?<type>v)=(?<id>\\d+)@','@facebook\\.com/(?<user>[.\\w]+)/(?<type>v)ideos/(?<id>\\d+)@'],'header'=>'User-agent: PHP (not Mozilla)','match'=>['@fb\\.watch/.@']]]],
		...
		'imdb'=>[[],[],[['extract'=>['!imdb\\.com/[/\\w]+?/vi(?<id>\\d+)/[/\\w]*?embed!'],'match'=>["!imdb\\.com/[/\\w]+?/vi(?'id'\\d+)!"],'url'=>'https://www.imdb.com/video/imdb/vi{@id}/imdb/embed']]],
		...
		'mastodon'=>[['#//(?<host>[-.\\w]+)/@(?<name>\\w+)/(?<id>\\d+)#'],['host'],[['extract'=>['#"url":"https://(?<host>[-.\\w]+)/@(?<name>\\w+)/(?<id>\\d+)"#'],'match'=>["#^(?'origin'https://[^/]+)/@\\w+@[-.\\w]+/(?'id'\\d+)#"],'url'=>'{@origin}/api/v1/statuses/{@id}']],['host'=>['s9e\\MediaSites\\Helper::filterMastodonHost']]],
		...
		'spotify'=>[['!(?:open|play)\\.spotify\\.com/(?:user/[-.\\w]+/)?(?<id>(?:album|artist|episode|playlist|show|track)(?:[:/][-.\\w]+)+)!'],[],[['extract'=>['!(?:open|play)\\.spotify\\.com/(?:user/[-.\\w]+/)?(?<id>(?:album|artist|episode|playlist|show|track)(?:[:/][-.\\w]+)+)!'],'header'=>'User-agent: PHP (not Mozilla)','match'=>['!link\\.tospotify\\.com/.!']]]],
		...
		'stitcher'=>[['!/(?:embed|splayer/f)/(?<fid>\\d+)/(?<eid>\\d+)!'],[],[['extract'=>['!/embed/(?<fid>\\d+)/(?<eid>\\d+)!'],'match'=>['!/(?:podcast/|show/|s\\?)!']]]],
		...
		'trendingviews'=>[['!(?:mydailyfreedom\\.com|trendingviews\\.co)/(?:tv/)?(?:embed|video)/(?:[^/]+-)?(?<id>\\d+)!']],
		'tumblr'=>[['!(?<name>[-\\w]+)\\.tumblr\\.com/post/(?<id>\\d+)!'],['did','key','name'],[['extract'=>['!did=(?:\\\\"|\\\\u0022)(?<did>[-\\w]+)!','!embed/post/t:(?<key>[-\\w]+)!'],'header'=>'User-agent: curl','match'=>['!\\w\\.tumblr\\.com/post/\\d!'],'url'=>'https://www.tumblr.com/oembed/1.0?url=https://{@name}.tumblr.com/post/{@id}']]],
		'twentyfoursevensports'=>[['!247sports\\.com/PlayerSport/[-\\w]*?(?<player_id>\\d+)/Embed!'],[],[['extract'=>['!247sports\\.com/PlayerSport/[-\\w]*?(?<player_id>\\d+)/Embed!'],'header'=>'User-agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.86 Safari/537.36','match'=>['!247sports\\.com/Player/[-\\w]*?\\d!']],['extract'=>['!embedVideoContainer_(?<video_id>\\d+)!'],'header'=>'User-agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.86 Safari/537.36','match'=>['!247sports\\.com/Video/.!']]]],
