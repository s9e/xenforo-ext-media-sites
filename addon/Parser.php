...
class Parser
{
	...
	*/
	protected static $sites = [
		...
		'facebook'=>[['@/(?!(?:apps|developers|graph)\\.)[-\\w.]*facebook\\.com/(?:[/\\w]+/permalink|(?!marketplace/|pages/|groups/).*?)(?:/|fbid=|\\?v=)(?<id>\\d+)(?=$|[/?&#])@','@facebook\\.com/(?<user>[.\\w]+)/(?=(?:post|video)s?/)(?<type>[pv])@','@facebook\\.com/video/(?=post|video)(?<type>[pv])@','@facebook\\.com/watch/\\?(?<type>[pv])=@','@facebook.com/groups/[^/]*/(?<type>p)osts/(?<id>\\d+)@'],[],[['extract'=>['@facebook\\.com/watch/\\?(?<type>v)=(?<id>\\d+)@','@facebook\\.com/(?<user>[.\\w]+)/(?<type>v)ideos/(?<id>\\d+)@'],'header'=>'User-agent: PHP (not Mozilla)','match'=>['@fb\\.watch/.@']]]],
		...
		'getty'=>[['!gty\\.im/(?<id>\\d+)!','!gettyimages\\.[.\\w]+/detail(?=/).*?/(?<id>\\d+)!','!#[-\\w]*picture-id(?<id>\\d+)$!'],[],[['extract'=>['!"height":[ "]*(?<height>\\d+)!','!"width":[ "]*(?<width>\\d+)!','!\\bid[=:][\'"]?(?<et>[-=\\w]+)!','!\\bsig[=:][\'"]?(?<sig>[-=\\w]+)!'],'match'=>['//'],'url'=>'https://embed.gettyimages.com/preview/{@id}']],['height'=>['s9e\\MediaSites\\Helper::filterUint'],'width'=>['s9e\\MediaSites\\Helper::filterUint']]],
		...
		'tiktok'=>[['#tiktok\\.com/(?:@[.\\w]+/video|v|i18n/share/video)/(?<id>\\d+)#'],[],[['extract'=>['#tiktok\\.com/(?:@[.\\w]+/video|v|i18n/share/video)/(?<id>\\d+)#'],'header'=>'User-agent: Mozilla/5.0 (X11; Linux x86_64; rv:62.0) Gecko/20100101 Firefox/62.0','match'=>['#//vm\\.tiktok\\.com/.#']]]],
