...
class Helper
{
	...
	*/
	public static function replaceIframes(Templater $templater, $type, $template, &$output)
	{
		...
		$output .= '<script>((e,h,g,u)=>{function w(a){["click","load","resize","scroll","visibilitychange"].forEach(b=>a(b,G,{capture:!0,passive:!0}))}function G(){e.clearTimeout(x);x=e.setTimeout(y,32)}function z(a){let b=-1,c=h.createElement("iframe"),d=JSON.parse(a.getAttribute(g+"-iframe"));for(;++b<d.length;)c.setAttribute(d[b],d[++b]);c.loading="eager";b=a.parentNode;c.hasAttribute(g)||b.hasAttribute("style")||(b.className=u+"-inactive",b.onclick=H,c.addEventListener("transitionend",I));a.replaceWith(c);2==c.getAttribute(g+"-api")&&(c.onload=J,a=k[A(c.src)],"string"===typeof a&&B(c,a))}function J(a){const b=new MessageChannel,c=a.target,d=c.src;b.port1.onmessage=f=>{f=""+f.data;B(c,f);try{if(.1>Math.random()&&k instanceof Storage){var m=k.length||0;if(100<m)for(;0<=--m;){const l=k.key(m)||"";/^s9e\//.test(l)&&.5>Math.random()&&k.removeItem(l)}}k[A(d)]=f}catch{}};c.contentWindow.postMessage("s9e:init","*",[b.port2])}function K(a){a=a.getBoundingClientRect();if(a.bottom>e.innerHeight)return 2;let b=-1;!C&&location.hash&&(b=n(location.hash,"top"));0>b&&(b=n(".p-navSticky","bottom"));return a.top<b?0:1}function n(a,b){return(a=h.querySelector(a))?a.getBoundingClientRect()[b]:-1}function B(a,b){b=b.split(" ");L(a,b[0],b[1]||0)}function L(a,b,c){const d=K(a),f=0===d||1===d&&1===p,m=f?n("html","height")-e.scrollY:0,l=a.style;if(1!==d||f||"complete"!==h.readyState)l.transition="none",e.setTimeout(()=>{l.transition=""},32);l.height=b+"px";c&&(l.width=c+"px");f&&((a=n("html","height")-e.scrollY-m)&&e.scrollBy(0,a),q=e.scrollY)}function y(){if("hidden"!==h.visibilityState){q===e.scrollY?p=0:(C=!0,p=q>(q=e.scrollY)?1:0);"complete"===h.readyState&&(v=2*e.innerHeight,D=-v/(0===p?4:2));var a=[];r.forEach(b=>{var c=b.getBoundingClientRect();if(c.bottom<D||c.top>v||!c.width)c=!1;else{let d=b.parentElement,f=d;for(;d;)/bbCodeBlock-expandContent/.test(d.className)&&(f=d),d=d.parentElement;c=c.top<=f.getBoundingClientRect().bottom}c?b.hasAttribute(g+"-c2l")?M(b):z(b):a.push(b)});r=a;r.length||w(e.removeEventListener)}}function H(a){a=a.target;const b=a.firstChild,c=a.getBoundingClientRect(),d=h.documentElement,f=b.style;f.bottom=d.clientHeight-c.bottom+"px";f.height=c.height+"px";f.right=d.clientWidth-c.right+"px";f.width=c.width+"px";b.offsetHeight&&/inactive/.test(a.className)?(a.className=u+"-active-tn",b.removeAttribute("style"),t&&t.click(),t=a):(a.className=u+"-inactive-tn",t=null)}function I(a){a=a.target;const b=a.parentElement;/-tn/.test(b.className)&&(b.className=b.className.replace("-tn",""),a.removeAttribute("style"))}function M(a){a.hasAttribute(g+"-c2l-background")&&((a.hasAttribute(g)?a:a.parentElement.parentElement).style.background=a.getAttribute(g+"-c2l-background"));a.onclick=b=>{b.stopPropagation();z(a)}}function A(a){return a.replace(/.*?ifram(e\/\d+\/\w+)[^#]*(#[^#]+).*/,"s9$1$2")}let E=h.querySelectorAll("span["+g+"-iframe]"),F=0,r=[],D=0,v=e.innerHeight,x=0,C=!1,q=e.scrollY,p=0,t=null,k={};for(;F<E.length;)r.push(E[F++]);try{k=e.localStorage}catch{}w(e.addEventListener);y()})(window,document,"data-s9e-mediaembed","s9e-miniplayer")</script>';
	}
