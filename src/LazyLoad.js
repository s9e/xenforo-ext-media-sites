((window, document, hash, dataPrefix, classPrefix) =>
{
	...
	*/
	function getLoadingRange()
	{
		...
		// If the page has not finished loading, we limit to the target range on first view, or
		// visible range if the user started scrolling during the "interactive" state
		return (hasScrolled) ? getVisibleRange() : getTargetRange();
	}
