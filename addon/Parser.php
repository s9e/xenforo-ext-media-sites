...
class Parser
{
	...
	*/
	protected static $sites = [
		...
		'xenforo'=>[['!^(?<url>https://.*?/)media/albums/(?:[-\\w]+\\.)?(?<xfmg_album_id>\\d+)!','!^(?<url>https://.*?/)(?:members/[-.\\w]+/#profile-post-|profile-posts/)(?<profile_post_id>\\d+)!','!^(?<url>https://.*?/)resources/(?:[-\\w]+\\.)?(?<resource_id>\\d+)!','!^(?<url>https://.*?/)threads/(?:[-\\w]+\\.)?(?<thread_id>\\d+)/(?:page-\\d+)?#?(?:post-(?<post_id>\\d+))?!','!^(?<url>https://.*?/)embed\\.php\\?content=(?<content_id>[-\\w]+)!'],['url'],[],['content_id'=>['s9e\\MediaSites\\Helper::filterIdentifier'],'post_id'=>['s9e\\MediaSites\\Helper::filterUint'],'profile_post_id'=>['s9e\\MediaSites\\Helper::filterUint'],'resource_id'=>['s9e\\MediaSites\\Helper::filterUint'],'thread_id'=>['s9e\\MediaSites\\Helper::filterUint'],'url'=>['s9e\\MediaSites\\Helper::filterUrl'],'xfmg_album_id'=>['s9e\\MediaSites\\Helper::filterUint']]],
