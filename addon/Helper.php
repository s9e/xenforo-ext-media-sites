...
class Helper
{
	...
	*/
	public static function replaceIframes(Templater $templater, $type, $template, &$output)
	{
		...
		$output .= '<script>((e,k,f,t)=>{function x(a){["click","load","resize","scroll","visibilitychange"].forEach(b=>a(b,H,{capture:!0,passive:!0}))}function H(){e.clearTimeout(y);y=e.setTimeout(z,32)}function A(a){let b=-1,c=k.createElement("iframe"),d=JSON.parse(a.getAttribute(f+"-iframe"));for(;++b<d.length;)c.setAttribute(d[b],d[++b]);c.loading="eager";I(c,a.parentNode);a.replaceWith(c);2==c.getAttribute(f+"-api")&&(c.onload=J,a=g[B(c.src)],"string"===typeof a&&C(c,a))}function J(a){const b=new MessageChannel,c=a.target,d=B(c.src);b.port1.onmessage=h=>{const l=""+h.data;e.setTimeout(()=>{C(c,g[d]||l)},c.getBoundingClientRect().height>+l.split(" ")[0]?5E3:0);try{.1>Math.random()&&K(),g[d]=l}catch{}};c.contentWindow.postMessage("s9e:init","*",[b.port2])}function L(a){a=a.getBoundingClientRect();if(a.bottom>e.innerHeight)return 2;let b=-1;!D&&location.hash&&(b=m(location.hash,"top"));0>b&&(b=m(".p-navSticky","bottom"));return a.top<b?0:1}function m(a,b){return(a=k.querySelector(a))?a.getBoundingClientRect()[b]:-1}function C(a,b){b=b.split(" ");M(a,b[0],b[1]||0)}function M(a,b,c){const d=L(a),h=0===d||1===d&&1===n,l=h?m("html","height")-e.scrollY:0,p=a.style;if(1!==d||h||"complete"!==k.readyState)p.transition="none",e.setTimeout(()=>{p.transition=""},32);p.height=b+"px";c&&(p.width=c+"px");h&&((a=m("html","height")-e.scrollY-l)&&e.scrollBy(0,a),q=e.scrollY)}function z(){if("hidden"!==k.visibilityState){q===e.scrollY?n=0:(D=!0,n=q>(q=e.scrollY)?1:0);"complete"===k.readyState&&(u=2*e.innerHeight,E=-u/(0===n?4:2));var a=[];r.forEach(b=>{var c=b.getBoundingClientRect();if(c.bottom<E||c.top>u||!c.width)c=!1;else{let d=b.parentElement,h=d;for(;d;)/bbCodeBlock-expandContent/.test(d.className)&&(h=d),d=d.parentElement;c=c.top<=h.getBoundingClientRect().bottom}c?b.hasAttribute(f+"-c2l")?N(b):A(b):a.push(b)});r=a;r.length||x(e.removeEventListener)}}function N(a){a.hasAttribute(f+"-c2l-background")&&((a.hasAttribute(f)?a:a.parentElement.parentElement).style.background=a.getAttribute(f+"-c2l-background"));a.onclick=b=>{b.stopPropagation();A(a)}}function I(a,b){a.hasAttribute(f)||b.hasAttribute("style")||(b.className=t+"-inactive",b.onclick=()=>{const c=b.getBoundingClientRect(),d=a.style;d.bottom=v.clientHeight-c.bottom+"px";d.height=c.height+"px";d.width=c.width+"px";"rtl"===v.dir?d.left=c.left+"px":d.right=v.clientWidth-c.right+"px";a.offsetHeight&&/inactive/.test(b.className)?(b.className=t+"-active-tn",a.removeAttribute("style"),w?.click(),w=b):(b.className=t+"-inactive-tn",w=null)},a.addEventListener("transitionend",()=>{/-tn/.test(b.className)&&(b.className=b.className.replace("-tn",""),a.removeAttribute("style"))}))}function B(a){return a.replace(/.*?ifram(e\/\d+\/\w+)[^#]*(#[^#]+).*/,"s9$1$2")}function K(){if(g instanceof Storage){var a=g.length||0;if(100<a)for(;0<=--a;){const b=g.key(a)||"";/^s9e\//.test(b)&&.5>Math.random()&&g.removeItem(b)}}}let F=k.querySelectorAll("span["+f+"-iframe]"),G=0,r=[],E=0,u=e.innerHeight,y=0,D=!1,q=e.scrollY,n=0,w=null,v=k.documentElement,g={};for(;G<F.length;)r.push(F[G++]);try{g=e.localStorage}catch{}x(e.addEventListener);z()})(window,document,"data-s9e-mediaembed","s9e-miniplayer")</script>';
	}
