	...
	*/
	public function transpile($template)
	{
		...
		$template = str_replace(
			...
			"((\$xf.versionId > 2030000 && \$xf.style.isVariationsEnabled()) ? ((\$xf.visitor.style_variation) ? property_variation('styleType', \$xf.visitor.style_variation) : 'auto') : property('styleType'))", $template
		);
	...
	*/
	protected function convertXPath($expr): string
	{
		...
		$replacements = [
			...
			"(^\\$(\\w+)(!=|(=))('[^']*') ?or ?\\$(\\w+)(!=|(=))('[^']*')$)D" => '$xf.options.s9e_MediaSites_$1$2$3$4 or $xf.options.s9e_MediaSites_$5$6$7$8',
			...
			"(^contains\\(@(\\w+)(,'[^']+')\\)$)D"              => '$$1 and contains($$1$2)',
			"(^not\\(contains\\(@(\\w+)(,'[^']+')\\)\\)$)D"     => '!contains($$1$2)',
