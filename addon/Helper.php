...
class Helper
{
	...
	*/
	public static function replaceIframes(Templater $templater, $type, $template, &$output)
	{
		...
		$output .= '<script>((e,l,g,u)=>{function w(a){["click","load","resize","scroll"].forEach(b=>a(b,H,{capture:!0,passive:!0}))}function H(){e.clearTimeout(x);x=e.setTimeout(y,32)}function z(a){let b=-1,c=l.createElement("iframe"),d=JSON.parse(a.getAttribute(g+"-iframe"));for(;++b<d.length;)c.setAttribute(d[b],d[++b]);c.loading="eager";b=a.parentNode;c.hasAttribute(g)||b.hasAttribute("style")||(b.className=u+"-inactive",b.onclick=I,c.addEventListener("transitionend",J));a.replaceWith(c);2==c.getAttribute(g+"-api")&&(c.onload=K,a=k[A(c.src)],"string"===typeof a&&(a=a.split(" "),B(c,a[0],a[1]||0)))}function K(a){const b=new MessageChannel,c=a.target,d=c.src;c.contentWindow.postMessage("s9e:init","*",[b.port2]);b.port1.onmessage=f=>{f=""+f.data;const m=f.split(" ");B(c,m[0],m[1]||0);try{if(.1>Math.random()&&k instanceof Storage){var h=k.length||0;if(100<h)for(;0<=--h;){const C=k.key(h)||"";/^s9e\//.test(C)&&.5>Math.random()&&k.removeItem(C)}}k[A(d)]=f}catch{}}}function L(a){a=a.getBoundingClientRect();if(a.bottom>e.innerHeight)return 2;let b=-1;!D&&location.hash&&(b=n(location.hash,"top"));0>b&&(b=n(".p-navSticky","bottom"));return a.top<b?0:1}function n(a,b){return(a=l.querySelector(a))?a.getBoundingClientRect()[b]:-1}function B(a,b,c){const d=L(a),f=0===d||1===d&&1===p,m=f?n("html","height")-e.scrollY:0,h=a.style;if(1!==d||f||"complete"!==l.readyState)h.transition="none",e.setTimeout(()=>{h.transition=""},32);h.height=b+"px";c&&(h.width=c+"px");f&&((a=n("html","height")-e.scrollY-m)&&e.scrollBy(0,a),q=e.scrollY)}function y(){q===e.scrollY?p=0:(D=!0,p=q>(q=e.scrollY)?1:0);"complete"===l.readyState&&(v=2*e.innerHeight,E=-v/(0===p?4:2));const a=[];r.forEach(b=>{var c=b.getBoundingClientRect();if(c.bottom<E||c.top>v||!c.width)c=!1;else{let d=b.parentElement,f=d;for(;d;)/bbCodeBlock-expandContent/.test(d.className)&&(f=d),d=d.parentElement;c=!(c.top>f.getBoundingClientRect().bottom)}c?b.hasAttribute(g+"-c2l")?M(b):z(b):a.push(b)});r=a;r.length||w(e.removeEventListener)}function I(a){a=a.target;const b=a.firstChild,c=a.getBoundingClientRect(),d=l.documentElement,f=b.style;f.bottom=d.clientHeight-c.bottom+"px";f.height=c.height+"px";f.right=d.clientWidth-c.right+"px";f.width=c.width+"px";b.offsetHeight&&/inactive/.test(a.className)?(a.className=u+"-active-tn",b.removeAttribute("style"),t&&t.click(),t=a):(a.className=u+"-inactive-tn",t=null)}function J(a){a=a.target;const b=a.parentElement;/-tn/.test(b.className)&&(b.className=b.className.replace("-tn",""),a.removeAttribute("style"))}function M(a){a.hasAttribute(g+"-c2l-background")&&((a.hasAttribute(g)?a:a.parentElement.parentElement).style.background=a.getAttribute(g+"-c2l-background"));a.onclick=b=>{b.stopPropagation();z(a)}}function A(a){return"s9e/"+a.replace(/.*?iframe\/(\d+\/\w+)[^#]*(#[^#]+)(?:#.*)?/,"$1$2")}let F=l.querySelectorAll("span["+g+"-iframe]"),G=0,r=[],E=0,v=e.innerHeight,x=0,D=!1,q=e.scrollY,p=0,t=null,k={};for(;G<F.length;)r.push(F[G++]);try{k=e.localStorage}catch{}w(e.addEventListener);y()})(window,document,"data-s9e-mediaembed","s9e-miniplayer")</script>';
	}
