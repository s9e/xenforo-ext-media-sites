...
class Helper
{
	...
	*/
	public static function replaceIframes(Templater $templater, $type, $template, &$output)
	{
		...
		$output .= '<script>((d,h,r,k,t)=>{function x(a){["click","load","resize","scroll","visibilitychange"].forEach(b=>a(b,H,{capture:!0,passive:!0}))}function y(){if(r){let a=h.querySelector(r)?.getBoundingClientRect().top??0;return[a,a+d.innerHeight]}return u()}function u(){return[Math.max(0,h.querySelector(".p-navSticky")?.getBoundingClientRect().bottom),d.innerHeight]}function H(){d.clearTimeout(z);z=d.setTimeout(A,32)}function B(a){let b=-1,c=h.createElement("iframe"),e=JSON.parse(a.getAttribute(k+"-iframe"));for(;++b<e.length;)c.setAttribute(e[b],e[++b]);c.loading="eager";I(c,a.parentNode);a.replaceWith(c);2==c.getAttribute(k+"-api")&&(c.onload=()=>J(c),a=l[C(c.src)],"string"===typeof a&&D(c,a))}function J(a){const b=new MessageChannel,c=C(a.src);b.port1.onmessage=e=>{const f=""+e.data;d.setTimeout(()=>{D(a,l[c]||f)},a.getBoundingClientRect().height>+f.split(" ")[0]?5E3:0);try{.1>Math.random()&&K(),l[c]=f}catch{}};a.contentWindow.postMessage("s9e:init","*",[b.port2])}function L(a){a=a.getBoundingClientRect();const [b,c]=u();return a.bottom>c?2:a.top<b?0:1}function D(a,b){const [c,e]=[...b.split(" "),0],f=a.style;if(f.height!==c+"px"||e&&f.width!==e+"px"){a=L(a);var g=(b=0===a||1===a&&1===n)?p.scrollHeight-d.scrollY:0;if(1!==a||b||"complete"!==h.readyState)f.transition="none",d.setTimeout(()=>{f.transition=""},32);f.height=c+"px";e&&(f.width=e+"px");b&&((a=p.scrollHeight-d.scrollY-g)&&d.scrollBy({behavior:"instant",top:a}),q=d.scrollY)}}function A(){q===d.scrollY?n=0:(E=!0,n=q>(q=d.scrollY)?1:0);if("hidden"!==h.visibilityState){var a=F;if(E)if("complete"!==h.readyState)var b=u();else b=2*d.innerHeight,b=[-b/(0===n?4:2),b];else b=y();a(b)}}function F(a){const b=[],[c,e]=a;v.forEach(f=>{var g=f.getBoundingClientRect();if(g.bottom<c||g.top>e||!g.width)g=!1;else{let m=f.parentElement,G=m;for(;m;)/bbCodeBlock-expandContent/.test(m.className)&&(G=m),m=m.parentElement;g=g.top<=G.getBoundingClientRect().bottom}g?f.hasAttribute(k+"-c2l")?M(f):B(f):b.push(f)});v=b;v.length||x(d.removeEventListener)}function M(a){a.hasAttribute(k+"-c2l-background")&&((a.hasAttribute(k)?a:a.parentElement.parentElement).style.background=a.getAttribute(k+"-c2l-background"));a.onclick=b=>{b.stopPropagation();B(a)}}function I(a,b){a.hasAttribute(k)||b.hasAttribute("style")||(b.className=t+"-inactive",b.onclick=()=>{const c=b.getBoundingClientRect(),e=a.style;e.bottom=p.clientHeight-c.bottom+"px";e.height=c.height+"px";e.width=c.width+"px";"rtl"===p.dir?e.left=c.left+"px":e.right=p.clientWidth-c.right+"px";a.offsetHeight&&/inactive/.test(b.className)?(b.className=t+"-active-tn",a.removeAttribute("style"),w?.click(),w=b):(b.className=t+"-inactive-tn",w=null)},a.addEventListener("transitionend",()=>{/-tn/.test(b.className)&&(b.className=b.className.replace("-tn",""),a.removeAttribute("style"))}))}function C(a){return a.replace(/.*?ifram(e\\/\\d+\\/\\w+)[^#]*(#[^#]+).*/,"s9$1$2")}function K(){if(l instanceof Storage){var a=l.length||0;if(100<a)for(;0<=--a;){const b=l.key(a)||"";/^s9e\\//.test(b)&&.5>Math.random()&&l.removeItem(b)}}}let w=null,p=h.documentElement,E=!1,q=d.scrollY,l={},v=[...h.querySelectorAll("span["+k+"-iframe]")],n=0,z=0;try{l=d.localStorage}catch{}A();x(d.addEventListener);d.navigation?.addEventListener("navigate",a=>{a=a.destination;a.sameDocument&&(a=/#.*/.exec(a.url))&&(r=a[0],n=q=0,F(y()))})})(window,document,location.hash,"data-s9e-mediaembed","s9e-miniplayer")</script>';
	}
