...
class Helper
{
	...
	*/
	public static function replaceIframes(Templater $templater, $type, $template, &$output)
	{
		...
		$output .= '<script>((d,h,t,k,u)=>{function y(a){["click","load","resize","scroll","visibilitychange"].forEach(b=>a(b,H,{capture:!0,passive:!0}))}function z(){if(t){let a=h.querySelector(t)?.getBoundingClientRect().top??0;return[a,a+d.innerHeight]}return v()}function v(){return[Math.max(0,h.querySelector(".p-navSticky")?.getBoundingClientRect().bottom),d.innerHeight]}function H(a){"click"===a.type&&"A"===a.target.tagName&&(n=!0);d.clearTimeout(A);A=d.setTimeout(B,32)}function C(a){let b=-1,c=h.createElement("iframe"),e=JSON.parse(a.getAttribute(k+"-iframe"));for(;++b<e.length;)c.setAttribute(e[b],e[++b]);c.loading="eager";I(c,a.parentNode);a.replaceWith(c);2==c.getAttribute(k+"-api")&&(c.onload=()=>J(c),a=l[D(c.src)],"string"===typeof a&&E(c,a))}function J(a){const b=new MessageChannel,c=D(a.src);b.port1.onmessage=e=>{const f=""+e.data;d.setTimeout(()=>{E(a,l[c]||f)},a.getBoundingClientRect().height>+f.split(" ")[0]?5E3:0);try{.1>Math.random()&&K(),l[c]=f}catch{}};a.contentWindow.postMessage("s9e:init","*",[b.port2])}function L(a){a=a.getBoundingClientRect();const [b,c]=v();return a.bottom>c?2:a.top<b?0:1}function E(a,b){const [c,e]=[...b.split(" "),0],f=a.style;if(f.height!==c+"px"||e&&f.width!==e+"px"){a=L(a);var g=(b=!n&&(0===a||1===a&&1===r))?q.scrollHeight-d.scrollY:0;if(1!==a||b||n||"complete"!==h.readyState)f.transition="none",d.setTimeout(()=>{f.transition=""},32);m=d.scrollY;f.height=c+"px";e&&(f.width=e+"px");m!==d.scrollY&&(m=d.scrollY,b=!1);b&&((a=q.scrollHeight-d.scrollY-g)&&d.scrollBy({behavior:"instant",top:a}),m=d.scrollY)}}function B(){var a=-1!==m;n?(n=!1,m=d.scrollY,r=0):r=m>(m=d.scrollY)?1:0;if("hidden"!==h.visibilityState){var b=F;a?"complete"!==h.readyState?a=v():(a=2*d.innerHeight,a=[-a/(0===r?4:2),a]):a=z();b(a)}}function F(a){const b=[],[c,e]=a;w.forEach(f=>{var g=f.getBoundingClientRect();if(g.bottom<c||g.top>e||!g.width)g=!1;else{let p=f.parentElement,G=p;for(;p;)/bbCodeBlock-expandContent/.test(p.className)&&(G=p),p=p.parentElement;g=g.top<=G.getBoundingClientRect().bottom}g?f.hasAttribute(k+"-c2l")?M(f):C(f):b.push(f)});w=b;w.length||y(d.removeEventListener)}function M(a){a.hasAttribute(k+"-c2l-background")&&((a.hasAttribute(k)?a:a.parentElement.parentElement).style.background=a.getAttribute(k+"-c2l-background"));a.onclick=b=>{b.stopPropagation();C(a)}}function I(a,b){a.hasAttribute(k)||b.hasAttribute("style")||(b.className=u+"-inactive",b.onclick=()=>{const c=b.getBoundingClientRect(),e=a.style;e.bottom=q.clientHeight-c.bottom+"px";e.height=c.height+"px";e.width=c.width+"px";"rtl"===q.dir?e.left=c.left+"px":e.right=q.clientWidth-c.right+"px";a.offsetHeight&&/inactive/.test(b.className)?(b.className=u+"-active-tn",a.removeAttribute("style"),x?.click(),x=b):(b.className=u+"-inactive-tn",x=null)},a.addEventListener("transitionend",()=>{/-tn/.test(b.className)&&(b.className=b.className.replace("-tn",""),a.removeAttribute("style"))}))}function D(a){return a.replace(/.*?ifram(e\\/\\d+\\/\\w+)[^#]*(#[^#]+).*/,"s9$1$2")}function K(){if(l instanceof Storage){var a=l.length||0;if(100<a)for(;0<=--a;){const b=l.key(a)||"";/^s9e\\//.test(b)&&.5>Math.random()&&l.removeItem(b)}}}let x=null,q=h.documentElement,n=!1,m=-1,l={},w=[...h.querySelectorAll("span["+k+"-iframe]")],r=0,A=0;try{l=d.localStorage}catch{}B();y(d.addEventListener);d.navigation?.addEventListener("navigate",a=>{a=a.destination;a.sameDocument&&(a=/#.*/.exec(a.url))&&(t=a[0],n=!0,F(z()))})})(window,document,location.hash,"data-s9e-mediaembed","s9e-miniplayer")</script>';
	}
